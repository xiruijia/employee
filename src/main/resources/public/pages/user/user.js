bdtApp.controller('userController',function($scope, $stateParams, HttpService, UserService,ModalService) {
	...
	$scope.login = function() {
		...
		HttpService.post("/employee/login", $scope.user).then(
				function(res) {
					if (res.code == 0) {
						$scope.hideError = true;
						$scope.initUser();
	...
	$scope.changeEmployee = function(type, value, name, $event) {
		...
		ModalService.yesno({
			...
			HttpService.post("/employee/updateEmployee", data)
					.then(function(res) {
						$scope.initUser();
					})
		}, function() {
	...
	$scope.checkMobile = function() {
		...
		} else if (/[\w!#$%&'*+/=?^_`{|}~-]+(?:\.[\w!#$%&'*+/=?^_`{|}~-]+)*@(?:[\w](?:[\w-]*[\w])?\.)+[\w](?:[\w-]*[\w])?/
				.test($scope.user.name)) {
			// 邮箱
			$scope.user.type = 'email';
		} else if (/^(\d{6})(\d{4})(\d{2})(\d{2})(\d{3})([0-9]|X)$/
				.test($scope.user.name)) {
			// 身份证
			$scope.user.type = 'idcard';
		} else if (!(/[\u4e00-\u9fa5]/.test($scope.user.name) || /\s/
				.test($scope.user.name))) {
			// code
	...
	$scope.register = function() {
		HttpService.post("/employee/register", $scope.user)
				.then(function(res) {
					if (res.code == 0) {
						$scope.hideError = true;
						user = res.data;
	...
	$scope.showPassword = function() {
		$('input[ng-model="user.password"]').attr("type",
				"text");
	}
